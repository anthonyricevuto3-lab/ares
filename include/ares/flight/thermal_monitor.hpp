...
// Reads ITemperatureSensor and evaluates freshness. It does not change spacecraft mode.
...
template <core::Clock C> class ThermalMonitor {
public:
    using time_point = typename C::time_point;

    ThermalMonitor(const hardware::ITemperatureSensor<time_point>& source, C& clock,
                   core::Duration max_age)
        : source_(source), clock_(clock), max_age_(max_age) {}

    const hardware::TemperatureSample<time_point>& sample() {
    ...
    [[nodiscard]] const hardware::TemperatureSample<time_point>&
    latest_observation() const noexcept {
        return latest_observation_;
    }

    [[nodiscard]] const std::optional<hardware::TemperatureSample<time_point>>&
...
private:
    const hardware::ITemperatureSensor<time_point>& source_;
    C& clock_;
    core::Duration max_age_;
    hardware::TemperatureSample<time_point> latest_observation_{};
    std::optional<hardware::TemperatureSample<time_point>> last_usable_{};
