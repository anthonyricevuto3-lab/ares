...
A deadline miss activates `DeadlineMiss` for that task. An on-time completion clears it. An unusable deadline result holds. One miss does not change spacecraft mode.
...
Warning faults, including a stale sensor, are recorded on the first detection. Policy leaves `Nominal` alone until that record's consecutive count reaches 3 (`kWarningPersistence`). That third detection may request `Degraded`. Advisory faults, including deadline misses, do not request a mode change in this milestone. One active `Critical` fault, or a saturated registry, may request `SafeMode` on the first detection. A critical condition is dominant: a persistent warning beside it does not change the `SafeMode` decision. When the critical fault later clears and a persistent warning remains, policy does not request `Nominal`. The action stays `ContinueDegraded`. From `SafeMode` that action does not request a mode, because the mode table has no edge back to `Degraded`. Hysteresis is what keeps a battery reading from chattering; the critical response itself is not delayed.

Recovery actions are `None`, `ContinueDegraded`, and `EnterSafeMode`. The action is the recommendation. It is not proof the spacecraft entered that mode. `PolicyDecision` keeps `requested_mode` separate from `transition`. `requested_mode` is empty until the current mode has a legal edge. `transition` is empty until `FdirController::apply` records the mode machine's `TransitionStatus`. A rejected transition leaves the mode and the registry unchanged. The registry is not updated from that status. The mode machine remains authoritative.

When no critical condition is present and no warning has reached the persistence count, policy may request `Degraded -> Nominal`. An advisory fault does not block that return. `SafeMode` is not left automatically.

## Events

`FaultActivated` is published when a record becomes active, including reactivation. `FaultCleared` is published when it becomes inactive. `FaultUpdated` is published once, when an active warning's consecutive count reaches the persistence limit. Later repeats are not events. The registry remains the authoritative state. Mode changes still publish the existing `ModeChangedEvent`.

## What this milestone does not do

It does not restart a task, switch to a redundant sensor, roll back a checkpoint, or vote. It does not escalate a deadline miss into `Degraded`. It does not invent a fault type for a future timestamp or a time error. One mailbox window still does not retain every intermediate non-usable sample. It retains whether `Usable` occurred, and what the newest sample was.
