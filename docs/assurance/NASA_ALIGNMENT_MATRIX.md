...
| SWE-034 | 3.1.5 | Define and document software acceptance criteria. | Voluntarily Adopted | Project gates in `SOFTWARE_ASSURANCE_PLAN.md`. Measured results are in `VERIFICATION_REPORT.md`. They are not a NASA customer acceptance. |
...
| SWE-037 | 3.1.7 | Define milestones at which developer progress is reviewed and audited. | Partially Implemented | `git tag --list` shows `v0.2.1`, `v0.3.0`, `v0.4.0`, `v0.5.0`, `v0.6.0`, `v0.7.0`, and `v1.0.0`. `docs/RELEASE_NOTES.md` records those milestones. There is no `v0.1.0` tag. No NASA audit. |
...
| ID | Section | Paraphrase | Status | ARES evidence |
| --- | --- | --- | --- | --- |
| SWE-052 | 3.12.1 | Maintain bi-directional traceability among higher-level requirements, software requirements, hazards, design, code, verification, and nonconformances, with the depth depending on class. | Partially Implemented | `REQUIREMENTS_TRACEABILITY.md`. Class-based NASA depth is not assigned because ARES is not classified. |
...
| SWE-053 | 4.1.5 | Track and manage changes to software requirements. | Partially Implemented | Git history. The assurance requirements describe frozen 1.0.0 behavior. They do not change it. |
| SWE-054 | 4.1.6 | Identify inconsistencies among requirements, plans, and products and track them to closure. | Partially Implemented | Defect handling in `SOFTWARE_ASSURANCE_PLAN.md`. No open inconsistency required a flight-behavior change. |
...
| ID | Section | Paraphrase | Status | ARES evidence |
| --- | --- | --- | --- | --- |
| SWE-057 | 4.2.3 | Transform software requirements into a recorded software architecture. | Implemented | `docs/ARCHITECTURE.md`. |
...
| SWE-136 | 4.4.8 | Validate and accredit tools required to develop or maintain the software. | Not Implemented | `VERIFICATION_REPORT.md` records tool use and limits. **NASA tool accreditation not performed.** |

## Test, coverage, and regression

| ID | Section | Paraphrase | Status | ARES evidence |
| --- | --- | --- | --- | --- |
| SWE-065 | 4.5.2 | Establish and maintain test plans, procedures, tests, and reports. | Partially Implemented | Verification strategy in `SOFTWARE_ASSURANCE_PLAN.md`, the GoogleTest sources, and `VERIFICATION_REPORT.md`. |
...
| SWE-068 | 4.5.5 | Evaluate test results and record the evaluation. | Implemented | `VERIFICATION_REPORT.md`. |
...
| ID | Section | Paraphrase | Status | ARES evidence |
| --- | --- | --- | --- | --- |
| SWE-079 | 5.1.2 | Develop a software configuration management plan. | Voluntarily Adopted | Configuration management section of `SOFTWARE_ASSURANCE_PLAN.md`. |
| SWE-080 | 5.1.3 | Track and evaluate changes to software products. | Implemented | Git history. `v1.0.0` is the frozen baseline. |
| SWE-081 | 5.1.4 | Identify configuration items and versions to control. | Implemented | Controlled-item list in `SOFTWARE_ASSURANCE_PLAN.md`. |
...
| SWE-084 | 5.1.7 | Audit configuration items against the records that define them. | Partially Implemented | The post-release assessment compared the tree to tag `v1.0.0`. `VERIFICATION_REPORT.md` records the measured result. |
...
| SWE-201 | 5.5.1 | Track and maintain software nonconformances, including tool and ground-software defects. | Voluntarily Adopted | Defect handling in `SOFTWARE_ASSURANCE_PLAN.md` and `CONTRIBUTING.md`. No fabricated historical database. |
...
| SWE-204 | 5.5.4 | Perform process assessments for all high-severity nonconformances. | Voluntarily Adopted | Required when a Critical or High item is closed. None is open against the frozen flight code. |
| SWE-087 | 5.3.2 | Perform and report peer reviews or inspections of requirements, plans, selected design, code, and test procedures. | Not Implemented | No human NASA Formal Inspection was performed. An AI-assisted review is not that inspection. |
| SWE-088 | 5.3.3 | For each review, use a checklist, readiness and completion criteria, action tracking, and identified participants. | Deferred | No formal inspection package was executed. A future peer review can use the requirements, coding standard, tests, and traceability. |
| SWE-089 | 5.3.4 | Record measurements for each planned review or inspection. | Deferred | No inspection measurements exist. No formal inspection was held. |
