...
Names: `nominal`, `gps_stale`, `gps_unavailable`, `imu_invalid`, `low_battery`, `deadline_storm`, `mixed_faults`, `nav_restart`, `restart_fail`. `ares --list-scenarios` prints the same names with one-line descriptions. `nav_restart` is a 1-second navigation delay and is the successful restart demonstration. `restart_fail` holds that delay for 30 seconds and is the failed-restart demonstration. `deadline_storm` remains the 4-second delay. The default is `nominal`. An unknown name exits nonzero with `unknown scenario:` and does not start workers. A schedule that fails `load` returns `StartupFailed` before `start()`, so workers do not run and the previous engine schedule is left unchanged. The shipped scenarios do not overlap on one target. `--duration-ms` is still the run length. The desktop clock must actually reach the event times; the deterministic tests advance `ManualClock` instead.
